...
def load_single_h5(path, coords, joints, cycles=1):
    ff = h5py.File(path, 'r')
    data_list = []
    num_samples = len(ff.keys())
    time_steps = 0
    labels = np.zeros(num_samples)  # dummy

    for si in range(num_samples):
        key = list(ff.keys())[si]
        data_list.append(list(ff[key]))
        time_steps = max(time_steps, len(ff[key]))

    data = np.empty((num_samples, time_steps * cycles, joints * coords))
    for si in range(num_samples):
        tiled = np.tile(data_list[si], (int(np.ceil(time_steps / len(data_list[si]))), 1))
        for ci in range(cycles):
            data[si, time_steps * ci:time_steps * (ci + 1), :] = tiled[0:time_steps]

    data = np.reshape(data, (data.shape[0], data.shape[1], joints, coords))
    return data, labels
